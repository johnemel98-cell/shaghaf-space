...
app.get('/api/purchase-orders/:id', authenticateToken, (req, res) => {
  res.json({ error: 'Purchase order not found' });
});
...
app.put('/api/purchase-orders/:id/status', authenticateToken, (req, res) => {
  res.json({ message: 'Purchase order status updated successfully' });
});

app.post('/api/purchase-orders/:id/items', authenticateToken, (req, res) => {
  const newItem = {
    id: Date.now().toString(),
    purchase_order_id: req.params.id,
    quantity: req.body.quantity || 1,
    unit_cost: req.body.unit_cost || 0,
    total_cost: (req.body.quantity || 1) * (req.body.unit_cost || 0),
    created_at: new Date().toISOString()
  };
  res.status(201).json(newItem);
});

app.put('/api/purchase-orders/:orderId/items/:itemId', authenticateToken, (req, res) => {
  res.json({ message: 'Purchase order item updated successfully' });
});

app.delete('/api/purchase-orders/:orderId/items/:itemId', authenticateToken, (req, res) => {
